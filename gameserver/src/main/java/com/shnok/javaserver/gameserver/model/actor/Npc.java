...
import com.shnok.javaserver.gameserver.model.actor.ai.type.NpcAI;
import com.shnok.javaserver.gameserver.model.actor.cast.NpcCast;
import com.shnok.javaserver.gameserver.model.actor.instance.Door;
import com.shnok.javaserver.gameserver.model.actor.instance.FriendlyMonster;
...
import com.shnok.javaserver.gameserver.model.actor.status.NpcStatus;
...
import com.shnok.javaserver.gameserver.model.location.TeleportLocation;
...
import com.shnok.javaserver.gameserver.network.serverpackets.ExShowVariationCancelWindow;
import com.shnok.javaserver.gameserver.network.serverpackets.ExShowVariationMakeWindow;
...
public class Npc extends Creature
{
	...
	public boolean isLordOwner(Player player)
	{
		...
		return _residence != null && _residence.getOwnerId() == player.getClanId();
	}
	...
	public void lookNeighbor()
	{
		...
		forEachKnownTypeInRadius(Creature.class, getSeeRange(), creature ->
		{
	...
	public boolean canAutoAttack(Creature target, int range, boolean allowPeaceful)
	{
		...
		if (attackDesireForTarget != null && !attackDesireForTarget.getMoveToTarget() && getMove().maybeStartOffensiveFollow(target, getStatus().getPhysicalAttackRange()))
			return false;
		...
		if (this instanceof Guard)
		{
			...
			if (target instanceof Monster targetMonster && Config.GUARD_ATTACK_AGGRO_MOB)
				return targetMonster.isAggressive() && GeoEngine.getInstance().canSeeTarget(this, targetMonster);
